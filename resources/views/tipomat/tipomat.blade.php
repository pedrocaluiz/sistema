    ...
    <div class="col-md-12">
      <div class="box">
              ...
              <div class="col-sm-12">
                @if (count($tiposMat) > 0)
                  <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                    ...
                    <tbody>
                    @foreach ($tiposMat as $tp)
                      <tr>
                        <td>{{$tp->id}}</td>
                        <td>{{$tp->descricao}}</td>
                        <td> <i class="{{$tp->icone}}"></i></td>
                        ...
                        @endforeach
                        <td>
                          <a href="/tipomat/{{$tp->id}}/edit" class="btn btn=sm btn-primary acaoTxt">@lang('messages.edit')</a>
                          <a href="/tipomat/{{$tp->id}}/edit" class="btn btn=sm btn-primary acaoIcon"><i class="fa fa-edit"></i></a>
                          <a class="btn btn=sm btn-danger acaoTxt" href="/tipomat/{{$tp->id}}"
                             onclick="event.preventDefault();
                                     document.getElementById('delete-form-{{$tp->id}}').submit();">
                            @lang('messages.delete')
                          </a>
                          <a class="btn btn=sm btn-danger acaoIcon" href="/tipomat/{{$tp->id}}"
                             onclick="event.preventDefault();
                                     document.getElementById('delete-form-{{$tp->id}}').submit();">
                            <i class="fa fa-edit"></i>
                          </a>
                          <form id="delete-form-{{$tp->id}}" action="/tipomat/{{$tp->id}}" method="POST" style="display: none;">
                            @method('DELETE')
                            @csrf
                          </form>
                        </td>
        ...
        <div class="box-footer d-flex justify-content-center">
          <div class="col-md-2">
            <a href="{{route('tipomat.create')}}" type="button"  class="btn btn-primary botao" id="cadastro">
              <i class="fa fa-plus"></i> &nbsp;&nbsp;@lang('messages.type-mat')
            </a>
          </div>
        </div>
      </div>
